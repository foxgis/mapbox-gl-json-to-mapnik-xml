...
function operatorFilter(filter, callback) {
  ...
  var op = ops[filter[0]]
  if (op) {
    xml_filter = field_name + ops[filter[0]] + getValue(field_name,filter[2])
  } else {
  ...
  if (filter[0] == 'in') {
    for(var i=2;i<filter.length;i++){
      ...
      if (xml_filter.trim().length > 0) {
        xml_filter += ' or ' + field_name + ' = ' + getValue(field_name,filter_item)
      } else {
        xml_filter += field_name + ' = ' + getValue(field_name,filter_item)
      }
      count++
    ...
    for(var i=2;i<filter.length;i++){
      var filter_item=filter[i];
      if (xml_filter.trim().length > 0) {
        xml_filter += ' and ' + field_name + ' != ' + getValue(field_name,filter_item)
      } else {
        xml_filter += field_name + ' != ' + getValue(field_name,filter_item)
      }
      count++
...
function combiningFilter(filter, callback) {
  ...
  var xml_filter = ''
  var type = filter[0]
  if(type.toLowerCase()=='all'&&filter.length<3){
    callback(null,'');
  }
