 ...
 * DATE:2016-6-14
 * Parameters:
 **@layer ObjectThe mapbox-gl Layer object
 * Return:
 ** BoolenIf the layer is invisible,return true
 */
...
function bgTranslate(layer) {
  ...
  if (bg['@background-opacity'] && bg['@background-color']) {
    var v = parseInt(bg['@background-opacity'] * 256).toString(16)
...
function lineTranslate(layer, filedir) {
  if (invisisble(layer)) {
    ...
    var line_symbol = { LineSymbolizer: {} }
    line_symbol.LineSymbolizer['@spacing'] = 250
    var count = 0
    ...
    for (var p in gstyle) {
      ...
      if (key !== '') {
        ...
        //}
        if (key == '@file') {
          line_symbol.LinePatternSymbolizer[key] = 'url(' + path.join(filedir, gstyle[p].replace('{', '[').replace('}', ']')) + '.svg' + ')'
        } else if (key == '@offset') {
...
function fillTranslate(layer, filedir) {
  if (invisisble(layer)) {
    ...
    var gstyle = uti.extend(layer.paint, layer.layout)
    var fill_symbol = { PolygonSymbolizer: {}, LineSymbolizer: {} }
    var count = 0
    var counts = Object.getOwnPropertyNames(gstyle).length
    for (var p in gstyle) {
      //MarkersSymbolizer
      ...
      if (key !== '') {
        if (key == '@file') {
          fill_symbol.PolygonSymbolizer[key] = 'url(' + path.join(filedir, gstyle[p].replace('{', '[').replace('}', ']')) + '.svg' + ')'
        } else {
      ...
      if (gstyle['fill-outline-color']) {
        ...
        delete fill_symbol.LineSymbolizer
      }
      count++
      if (count >= counts) {
        return fill_symbol
    ...
    for (var p in gstyle) {
      ...
      if (p.indexOf('text') === -1) {
        if (p == 'icon-rotate') {
          ...
          fill_symbol.PolygonSymbolizer['@file'] = 'url(' + path.join(filedir, gstyle[p].replace('{', '[').replace('}', ']')) + '.svg' + ')'
        } else {
