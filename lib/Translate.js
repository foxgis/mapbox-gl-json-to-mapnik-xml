...
function fillTranslate(layer, filedir) {
    if (invisisble(layer)) {
        ...
        for (var p in gstyle) {
            ...
            if (key !== '') {
                if (key == '@file') {
                    fill_symbol.PolygonSymbolizer[key] = path.join(filedir, gstyle[p]).replace(/\\/g,'/')+ '.svg';;
                }
                else {
...
function symbolTranslate(layer, filedir) {
    if (invisisble(layer)) {
        ...
        var gstyle = uti.extend(layer.paint, layer.layout);
        var symbolizer = {TextSymbolizer: {}, MarkersSymbolizer: {},PolygonSymbolizer:{}};
        var count = 0;
        var counts = Object.getOwnPropertyNames(gstyle).length;
        for (var p in gstyle) {
            //MarkersSymbolizer
            if (p.indexOf('text') === -1) {
                if (p == 'icon-rotate') {
                    ...
                    symbolizer.PolygonSymbolizer['@file'] = path.join(filedir, gstyle[p]).replace(/\\/g,'/')+'.svg';
                }
                else {
